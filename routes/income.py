...
def get_trend_coordinator_attribution(course_enrollment_id):
    ...
    if not trend_attribution:
        raise exceptions.TrendCoordinatorNotFoundError()
...
def create_income_record(csv_row, for_month, fk_income_source_id, payment_method, payment_status, fk_bank_account_id,
                         institutions_branches_map,course_start_date, course_end_date, data_source):
    ...
    fk_trend_coordinator_id = trend_coordinator_attribution.get('fk_trend_coordinator_id')
    income_record = dict(for_month=for_month, amount=csv_row.get(AMOUNT_COL),
                         ...
                         fk_income_source_id=fk_income_source_id, fk_trend_coordinator_id=fk_trend_coordinator_id,
...
def create_income_from_csv(csv_reader, payment_month, fk_income_source_id, payment_method,
                           payment_status, fk_bank_account_id, data_source,update_previous=False):
    pop_up_results = []
...
class IncomeAPI(Resource):
    @login_required()
    def get(self, income_id=None, current_account_id=None):
        ...
        if income_id:
            results = Income.read(many=False, id=income_id)
        elif current_account_id:
            results = Income.read(many=False, fk_current_account_id=current_account_id)
        else:
            results = Income.read()
    ...
    def post(self):
        ...
        if request_path[-1] == "print_payment":
            ...
            zip_data, zip_name = Income.export_draft(user_json)
        ...
        if request.files and STUDENTS_INCOMES_CSV_PARAMETER in request.files:
            ...
            students_incomes_csv = StringIO(
                constants.NEW_LINE.join(students_incomes_csv_data[CSV_ROWS_TO_SKIP + 1:]))
            ...
            return jsonify(create_income_from_csv(csv_reader, payment_month,
                                                  incomes_parameters['fk_income_source_id'],
                                                  incomes_parameters['method'],
                                                  incomes_parameters['payment_status'], fk_bank_account_id))
        else:
            income_json = request.get_json()
            return jsonify(Income.create(income_json))

    @login_required()
    def put(self, income_id=None):
        # Add from csv file
        if request.files and STUDENTS_INCOMES_CSV_PARAMETER in request.files:
            ...
            return jsonify(create_income_from_csv(csv_reader, payment_month,
                                                  ...
                                                  incomes_parameters['method'],
                                                  incomes_parameters['payment_status'], fk_bank_account_id,
                                                  update_previous=True,data_source=data_source))
        income_json = request.get_json()
        return jsonify(Income.update(income_json, id=income_id))

    @login_required()
    def delete(self, income_id):
        result = Income.delete(id=income_id)

        if result[constants.STR_ERROR]:
            return jsonify({constants.STR_ERROR: True,
                            constants.STR_MESSAGE: messages.COLUMN_DELETE_FAILED.format("Income")})

        return jsonify({constants.STR_ERROR: False,
                        constants.STR_MESSAGE: messages.COLUMN_DELETE_SUCCESSFULLY.format("Income")})
