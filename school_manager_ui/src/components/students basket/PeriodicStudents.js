...
const PeriodicStudents = (props) => {
    ...
    return (
        <Container title='מצבת תלמידים' childrenStyle={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <div className='paymentReportTablesContainer' style={{
                ...
                <div style={{ height: periodicReceptionStore.rightTablePeriodicReception.length < 10 && 'fit-content', marginRight: 'calc(5em + 5px)' ,minHeight: '30em'}}>
                    ...
                    <Table header={['שם', 'סל תלמידים']}
                        ...
                        <div className='paymentReportTableTabsPosition' style={{ left: '100%' }}>
                            ...
                            <div className={`paymentReportRightTableTab ${periodicReceptionStore.selectedRightTableTab === 'trend_coordinator' && 'selectedTab'}`} onClick={e => HandleSelectedTableTab(e, 'trend_coordinator', 1, '.paymentReportRightTableTab')}>סניפים</div>
                            <div className={`paymentReportRightTableTab ${periodicReceptionStore.selectedRightTableTab === 'student' && 'selectedTab'}`} onClick={e => HandleSelectedTableTab(e, 'student', 1, '.paymentReportRightTableTab')}>מנחים</div>
                            <div className={`paymentReportRightTableTab ${periodicReceptionStore.selectedRightTableTab === 'donator' && 'selectedTab'}`} onClick={e => HandleSelectedTableTab(e, 'donator', 1, '.paymentReportRightTableTab')}>ללא</div>
                        </div>
                ...
                <div style={{ height: periodicReceptionStore.leftTablePeriodicReception.length < 10 && 'fit-content', marginLeft: '4em' ,minHeight: '30em'}}>
                    ...
                    <Table header={['סטטוס רישום', 'סיום-קורס', 'התחלה-קורס', 'סוג-קורס', 'מזהה', 'שם מלא', 'נייד', 'כתובת', 'גיל', 'דוא"ל', 'שיטת מלגה', 'שיטת תשלום', 'זמן ביום','שיטת זכאות','דרגות זכאות','סיום-מנחה','התחלה-מנחה']}
                        noEdit
                        // noFottTitle={['מזהה']}
...
export default inject('rootStore')(observer(PeriodicStudents));